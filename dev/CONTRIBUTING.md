...
This outlines how to propose a change to `hubExamples`. For more general
...
- Fork the package and clone onto your computer. If you haven’t done
  ...
  `usethis::create_from_github("hubverse-org/hubExamples", fork = TRUE)`.
...
- If you’re changing the scripts in `data-raw/`, you can run tests
  locally: `devtools::test()` (because the contents of `data-raw` are
  not included in the package, these tests are skipped during R checks).
...
Please note that the hubExamples project is released with a [Contributor
Code of
Conduct](https://hubverse-org.github.io/hubExamples/dev/CODE_OF_CONDUCT.md).
